...
# ADDED: Performance measurement arguments
parser.add_argument('--max-iters', default=None, type=int,
                    help='maximum number of training iterations (default: None = full epoch)')
parser.add_argument('--warmup-iters', default=10, type=int,
                    help='number of warmup iterations to skip before measurement (default: 10)')
parser.add_argument('--log-file', default='', type=str,
                    help='path to CSV log file for performance metrics')

best_acc1 = 0

# ADDED: Global CSV writer
csv_writer = None
csv_file = None
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # ADDED: Initialize CSV logging
    if args.log_file and (not args.distributed or args.rank == 0):
        csv_file = open(args.log_file, 'w', newline='')
        csv_writer = csv.writer(csv_file)
        csv_writer.writerow(['env', 'device', 'model', 'batch_size', 'iter_idx', 
                            'iter_time_ms', 'imgs_per_sec', 'timestamp'])
...
def train(train_loader, model, criterion, optimizer, epoch, device, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        # ADDED: Check if max_iters reached
        if args.max_iters is not None and i >= args.max_iters:
            print(f"\nReached max iterations ({args.max_iters}). Stopping training.")
            break
        ...
        # ADDED: End iteration timing
        iter_end_time = time.time()
        iter_time_ms = (iter_end_time - iter_start_time) * 1000
        imgs_per_sec = args.batch_size / (iter_end_time - iter_start_time)

        # measure elapsed time
        batch_time.update(time.time() - end)
        end = time.time()

        # ADDED: Log to CSV (skip warmup iterations)
        if csv_writer and i >= args.warmup_iters:
            device_name = 'cuda' if torch.cuda.is_available() else 'cpu'
            csv_writer.writerow([
                'envA',  # Environment (can be modified)
                device_name,
                args.arch,
                args.batch_size,
                i - args.warmup_iters,  # Adjusted iteration index (0-based after warmup)
                f'{iter_time_ms:.2f}',
                f'{imgs_per_sec:.2f}',
                time.time()
            ])
